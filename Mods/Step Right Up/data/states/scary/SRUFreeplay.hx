...
var wheelRotationSpeed:Float = 2;
...
var songRotationAngles = {
    'deep-dream': 30,
    'Mortis': 60,
    'minds-eye-squared': 90,
    'no-more': 120,
    '': 0 
};
...
function update() {
    ...
    //wheel.angle += wheelRotationSpeed;
}
function changeItem(huh: Int = 0) {
    ...
    if (curSelected >= 0 && curSelected < preloadedLogos.length && curSelected < preloadedAnims.length) {
        logo.loadGraphic(preloadedLogos[curSelected]);
        char.frames = Paths.getSparrowAtlas('freeplay/anims/' + Characters[curSelected]);
